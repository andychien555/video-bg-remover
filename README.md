...
頂部可切換 **影片去背 / 圖片去背** 兩種模式：圖片模式上傳單張圖（PNG / JPG / WebP…）→ 套用同一套去背參數 → **輸出單張透明背景 PNG**。

全部運算都在你的瀏覽器本機完成，影片**不會上傳到任何伺服器**，可直接部署到 GitHub Pages 等靜態空間。

> ⚠️ **MOV / WebM 例外**：透明 MOV 與壓縮 WebM 都用 `ffmpeg.wasm` 在瀏覽器內編碼，運算一樣在本機、影片不外流，但**首次匯出需從 CDN 下載約 30MB 的 ffmpeg 核心**（之後快取，MOV 與 WebM 共用同一份）。因此只有這兩種格式在執行期會連一次 CDN，其餘格式（WebP / GIF / PNG）皆零外部相依、可完全離線。

> 逐幀精確處理 · 30fps · 瀏覽器內合成動態 WebP

## ✨ 功能

- **影片 / 圖片雙模式**：頂部一鍵切換。圖片模式上傳單張圖即套用同一套去背參數與預覽，輸出單張透明 PNG（保留 alpha，附即時大小預估）。可選**無損壓縮**（像素不變、僅重新最佳化，通常比原生 PNG 小很多）或**有損調色盤量化**（≤256 色、保留透明度，檔案最小，可開 Floyd–Steinberg 抖色平滑色帶）。拖入圖片檔會自動切到圖片模式。
- **兩種去背模式**
...
- **壓縮透明 WebM**：在瀏覽器內用 **ffmpeg.wasm** 以 **VP9** 編碼保留 alpha 的 `.webm`，採 **CRF 固定品質**壓縮（`-b:v 0 -crf`）——是體積最小的透明影片格式，適合網頁、Chrome／Firefox／Edge。可調**輸出寬度**、**幀率**與 **CRF**（數字越大越小越糊）；首次使用會下載一次 ffmpeg 核心（與 MOV 共用，見上方說明）。按「**估算大小**」會實際用 VP9 編一段**連續取樣幀**再依幀數外插（VP9 有幀間壓縮，不能像 WebP 那樣抽散幀外插），給出編碼前的大小概估。
...
7. 設定輸出寬度與幀率後輸出：
   ...
   - 或切到 **GIF** 分頁 → 「轉成動態 GIF」。
   - 或切到 **WebM** 分頁 → 調寬度／幀率／CRF → 「壓縮並匯出 .webm」→「下載 WebM」。
...
│   ├── mov-encoder.js      # ffmpeg.wasm 編碼器（單執行緒核心，MOV + WebM 共用）：透明 MOV（ProRes 4444 / qtrle）與壓縮 WebM（VP9-alpha + CRF）
│   └── png-encoder.js      # 單張透明 PNG 壓縮（UPNG.js）：無損最佳化 / 有損調色盤量化 + Floyd–Steinberg 抖色
...
│   ├── pako.min.js         # 2.1.0（deflate，UPNG 相依）
│   └── UPNG.js             # 2.1.0（透明 PNG 壓縮／量化）
├── test/
│   ├── muxer-test.html     # 動態 WebP muxer 的瀏覽器自我測試
│   ├── pipeline-test.html  # keying.js + pipeline.js 的瀏覽器自我測試
│   └── png-test.html       # png-encoder.js 的瀏覽器自我測試（各壓縮模式大小 + 解碼驗證）
└── .nojekyll
```
...
| [UPNG.js](https://github.com/photopea/UPNG.js) | 2.1.0 | 透明 PNG 壓縮（無損最佳化 + 有損調色盤量化） | MIT |
| [pako](https://github.com/nodeca/pako) | 2.1.0 | deflate（UPNG.js 相依） | MIT / Zlib |

`@jsquash/webp` 內含 Google libwebp（BSD-3-Clause）編譯出的 WASM。

### ⚠️ MOV / WebM 例外：執行期載入的 ffmpeg.wasm

透明 MOV 與壓縮 WebM 用的 `ffmpeg.wasm` 體積很大（核心約 30MB），不適合 vendored，因此**只在首次匯出 MOV 或 WebM 時從 jsDelivr CDN lazy-load 並快取**（兩種格式共用同一份已載入的實例）。版本皆鎖定於 `js/mov-encoder.js` 中的常數：
...
>
> 影響：**只有 MOV 與 WebM 在執行期會連一次 CDN，且首次使用需連網**；其餘格式（WebP / GIF / PNG）完全零外部相依、可離線。
...
建議使用最新版 **Chrome / Edge**。動態 WebP 與 PNG 序列在各瀏覽器相容性最佳；含透明通道的 WebM（VP9-alpha）在 Chrome／Firefox／Edge 播放最穩、體積最小，是網頁透明影片的首選，但部分播放器（如 Safari）對 alpha WebM 支援不一。若要在網頁／預覽端透明度最穩可改用 **動態 WebP**；若要丟進剪輯軟體（Premiere／FCP／AE），用 **透明 MOV**。MOV 與 WebM 皆由 ffmpeg.wasm 在瀏覽器內編碼，首次使用需下載 ffmpeg 核心、且編碼（尤其 ProRes 4444）較耗時，屬正常；WebM 的 CRF 越大檔案越小、畫面越糊。
